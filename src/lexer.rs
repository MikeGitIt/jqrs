...
pub fn yy_pop_state(yyscanner: &mut YyGutsT) {
    ...
    if idx < yyscanner.yy_start_stack.len() {
        yyscanner.yy_start = 1 + 2 * yyscanner.yy_start_stack[idx];
    }
}
...
fn try_exit_bracket(c: i32, currstate: i32, yyscanner: &mut YyGutsT) -> i32 {
    ...
    // Pop state and return the closing bracket
    yy_pop_state(yyscanner);

    // Check if we're exiting interpolation back to string
    if yyscanner.yy_start == 1 + 2 * IN_QQSTRING {
